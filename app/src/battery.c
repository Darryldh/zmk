...
static char stringify_sensor_values(const char *unit, struct sensor_value value) {
    // combine multiple values, round to two decimals, apend units on the end
    static char buffer [20];

    if ((value.val2 < 0) && (value.val1 >= 0)) {
		value.val2 = -(value.val2);
		sprintf(buffer, "-%d.%02d%s", value.val1, value.val2, unit);
		...
		sprintf(buffer, "%d.%06d%s", value.val1, value.val2, unit);
	}
    return unit;
}
...
static void zmk_battery_update(const struct device *battery) {
    ...
    while (1) {
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->voltage = voltage.val1 + voltage.val2;
            //LOG_DBG("-----> battery voltage: %d.%dV", voltage.val1, voltage.val2);
            char formattedValue = stringify_sensor_values("V", voltage);
            ev->strVoltage = formattedValue;
            LOG_DBG("-----> battery voltage: %s", formattedValue);
        }
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->current = current.val1 + current.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->full_charge_capacity = full_charge_capacity.val1 + full_charge_capacity.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->remaining_charge_capacity = remaining_charge_capacity.val1 + remaining_charge_capacity.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->avg_power = avg_power.val1 + avg_power.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->int_temp = int_temp.val1 + int_temp.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->current_standby = current_standby.val1 + current_standby.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->current_max_load = current_max_load.val1 + current_max_load.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->int_ttempty = int_ttempty.val1 + int_ttempty.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->int_ttfull = int_ttfull.val1 + int_ttfull.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->nominal_capacity = nominal_capacity.val1 + nominal_capacity.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->designed_voltage = designed_voltage.val1 + designed_voltage.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->desired_voltage = desired_voltage.val1 + desired_voltage.val2;
        ...
        if (status == -ENOTSUP) {
            ...
            /* append this value to the event */
            ev->desired_charge_current = desired_charge_current.val1;
